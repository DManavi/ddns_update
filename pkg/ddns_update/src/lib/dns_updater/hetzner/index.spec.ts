...
describe('ddns_update | lib | dns_updater | hetzner', () => {
  // ENV variables
  const API_KEY = env('TEST_HETZNER_API_KEY').required().asString();
  const DOMAIN_NAME = env('TEST_HETZNER_DOMAIN_NAME').required().asString();
